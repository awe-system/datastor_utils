...
class algo_metadata_callback : public block_io_callback, public block_event_callback
{
};
...
public:
    ...
    virtual int regroup_pool(algo_device_list_t &algo_device_list, block_io_callback *io_callback)= 0;

    virtual int create_pool(const string &poolname, algo_pool_info *pool_info)= 0;

    virtual int remove_pool(const string &pool_name)= 0;

    virtual int extend_pool(const string &main_pool, const string &extra_pool)= 0;

    virtual int migrate_volume(const string& org_pool_name,const  string& org_vol_name,const  string& dest_pool_name,
...
class algo_pool_operation : public block_event_callback, public ref_obj, public algo_obj
{
private:
...
private:
    ...
    virtual void save(const json_obj &storage_info) =0;//池内信息的保存实际使用时直接伴组合to_json_obj使用
...
public:
    ...
    //卷和池相关的
    virtual int create_vol(const string &vol_name, unsigned long long size_sec, block_io_callback *io_callback) = 0;//包含精简配置
...
public:
    ...
    void split_super(const json_obj &whole_info,json_obj &memory_info,json_obj &storage_info);
