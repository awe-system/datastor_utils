...
algo_section::algo_section(unsigned long _start, unsigned long _len) : start(_start), len(_len)
{
}
...
void algo_section::split(const algo_section &small_sec, algo_section_vec_t &sections) const
{
    ...
    {
        sections.push_back(algo_section(small_sec.start + small_sec.len,start + len - small_sec.start - small_sec.len));
    }
}
...
{
    if(obj.type() != json_obj_type_array || obj.size() != 2) throw (int)-ServerSan_Algo::ERROR_TYPE_JSON_FORMAT;
    start = obj[0].ll_val;
    len = obj[1].ll_val;
}
...
{
    for ( algo_section_const_it it = sections.begin(); it != sections.end(); ++it )
    {
        it->dump();
        cout << "\t";
    }
