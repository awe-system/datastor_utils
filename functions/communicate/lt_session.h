...
#define DEFAULT_WAIT_SECONDS 3000
...
public:
    virtual void
    rcv_done(lt_session *sess, lt_data_t *received_data, int error) = 0;
    
    virtual void
    snd_done(lt_session *sess, lt_data_t *sent_data, int error) = 0;
...
class lt_session
        : public lt_reference,
          public lt_session_dog,
          public lt_session_description_imp,
...
private:
    ...
    void rcv_head_done_unsafe(lt_data_t *data,
                              const boost::system::error_code error);
    
    void start_rcv_data_unsafe(lt_data_t *data);
    
    void rcv_data_done_unsafe(lt_data_t *data,
                              const boost::system::error_code error);

private:
    void start_snd_data_unsafe(lt_data_t *data);
    
    void snd_data_done_unsafe(lt_data_t *data,
                              const boost::system::error_code &error);
