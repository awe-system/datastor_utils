...
namespace log4cpp
{
   ...
   std::unique_ptr<Layout> create_pattern_layout(const FactoryParams& params);
   std::unique_ptr<Layout> create_pass_through_layout(const FactoryParams& params);

   LayoutsFactory& LayoutsFactory::getInstance()
   {
      if (!layouts_factory_)
      {
         std::unique_ptr<LayoutsFactory> lf(new LayoutsFactory);
   ...
   std::unique_ptr<Layout> LayoutsFactory::create(const std::string& class_name, const params_t& params)
   {
      const_iterator i = creators_.find(class_name);
      if (i == creators_.end())
