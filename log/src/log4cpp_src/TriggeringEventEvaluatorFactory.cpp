...
namespace log4cpp
{
   ...
   TriggeringEventEvaluatorFactory& TriggeringEventEvaluatorFactory::getInstance()
   {
      ...
      {
         std::unique_ptr<TriggeringEventEvaluatorFactory> af(new TriggeringEventEvaluatorFactory);
   ...
   std::unique_ptr<TriggeringEventEvaluator> TriggeringEventEvaluatorFactory::create(const std::string& class_name, const params_t& params)
   {
      const_iterator i = creators_.find(class_name);
      if (i == creators_.end())
